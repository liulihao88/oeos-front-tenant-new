<template>
  <el-descriptions v-bind="{ border: true, ...$attrs }" :column="column" class="o-description">
    <el-descriptions-item v-for="(item, index) in options" :key="index">
      <template #label>
        <o-tooltip :content="item.label" />
      </template>
      <template v-if="item.slotName">
        <slot :name="item.slotName" :item="item" :label="item.label" :value="item.value" :index="index" />
      </template>
      <template v-else>
        <o-tooltip class="w-100%" :content="parseContent(item.value)" />
      </template>
